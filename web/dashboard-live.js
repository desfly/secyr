...
(() => {
  ...
  function renderLiveZones(states) {
    ...
    target.innerHTML = live.zones.map((state, index) => {
      ...
      return `<div class="zone ${view.cls}" data-zone-id="${index + 1}" data-zone-state="${state}"><span>${escape(name)}${meta.alwaysOn ? " · 24/7" : ""}</span><strong>${view.label}</strong></div>`;
    }).join("");
  }
  ...
  function replaceAndBindButton(id, handler) {
    ...
    if (oldButton.dataset.liveBound === "true") return true;
  ...
  function bindQuickButtons() {
    const lightBound = replaceAndBindButton("quickLight", async event => {
    ...
    const lockBound = replaceAndBindButton("quickLock", async event => {
    ...
    if (lightBound || lockBound) refreshOutputs();
  }
  ...
  async function openTelemetrySocket() {
    if (!window.HomeGuardAuth?.authenticated?.() || live.socket) return;
    const actor = window.HomeGuardAuth?.actor?.() || "";
    if (!actor) return;
    try {
      const ticket = await api("/api/v1/telemetry/session", {
        method: "POST",
        body: JSON.stringify({ actor })
      });
      const token = String(ticket?.telemetryToken || "");
      if (token.length < 32) throw new Error("telemetry ticket unavailable");
      const scheme = window.location.protocol === "https:" ? "wss" : "ws";
      const socket = new WebSocket(`${scheme}://${window.location.host}/ws/telemetry?ticket=${encodeURIComponent(token)}`);
      live.socket = socket;
      socket.onmessage = event => {
        try {
          const frame = JSON.parse(event.data);
          if (Array.isArray(frame?.zones)) renderLiveZones(frame.zones);
        } catch (_) {
        }
      };
      socket.onclose = () => {
        ...
        if (window.HomeGuardAuth?.authenticated?.()) {
          ...
          live.reconnectTimer = window.setTimeout(openTelemetrySocket, 1500);
        }
      };
      socket.onerror = () => {
      ...
      window.clearTimeout(live.reconnectTimer);
      live.reconnectTimer = window.setTimeout(openTelemetrySocket, 2000);
    }
  }
