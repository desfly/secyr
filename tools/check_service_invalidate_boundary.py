...
required = [
    ...
    'self->access_control_->authorize(actor, credential, "system.service.invalidate")',
...
rbac_pos = text.find('self->access_control_->authorize(actor, credential, "system.service.invalidate")')
erase_pos = text.find("self->store_->erase_all()")
if min(auth_pos, rbac_pos, erase_pos) < 0 or not (auth_pos < rbac_pos < erase_pos):
    errors.append("service invalidation must authenticate actor and authorize command before erase_all")
