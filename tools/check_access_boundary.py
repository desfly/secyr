...
# Public access lifecycle must expose only the minimal setup/login decision.
...
# Locked web UI must exist and must bootstrap only through the lifecycle endpoint.
require(WEB / "access-session.js", [
    "hg-auth-locked",
    "/api/v1/access/state",
    "/api/v1/access/login",
    "HomeGuard ${session.actor}:${session.credential}",
])
