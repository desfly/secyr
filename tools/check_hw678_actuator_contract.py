...
checks = {
    ...
    "bench target evidence separated from pulse success": "bool* target_evidence = nullptr" in physical_h and "*target_evidence = target_limit_reached" in physical_cpp,
    "service counts only confirmed evidence": "valve_mask != 0U && target_evidence" in service and "targetReached" in service and "pulseApplied" in service,
    ...
    "physical test checks repeatable partial pulse": "bool bench_evidence = true" in t54 and "TEST_CHECK(!bench_evidence)" in t54,
    "physical test checks end-stop transition evidence": "bench_transition_cold_open = true" in t54 and "TEST_CHECK(bench_evidence)" in t54,
    "physical test checks active target no-pulse": "bench_delay_seen == 0U" in t54,
