...
struct TelemetryFrame {
 ...
 std::array<ZoneState,8> zones{}; std::array<PressureState,2> pressures{}; std::array<float,2> pressure_values{}; std::array<bool,2> pressure_valid{}; HealthState health{HealthState::Unknown}; uint32_t failed_components{};
...
public:
 TelemetryFrame build(uint64_t uptime_ms, uint64_t rtc_epoch, SystemMode mode, Transport transport, const std::array<ZoneState,8>& zones, const std::array<PressureState,2>& pressures, const HealthMonitor& health, const std::array<float,8>& temperatures_c = {}, const std::array<bool,8>& temperature_valid = {}, uint8_t temperature_count = 0, float battery_voltage_v = 0.0F, float battery_current_a = 0.0F, float battery_power_w = 0.0F, bool battery_valid = false, const std::array<float,2>& pressure_values = {}, const std::array<bool,2>& pressure_valid = {});
