...
std::string telemetry_json(const TelemetryFrame& f) {
 ...
 out += ",\"battery_valid\":";
 out += f.battery_valid ? "true" : "false";
 out += ",\"crc\":" + std::to_string(f.crc) + "}";
 return out;
}
TelemetryFrame TelemetryBuilder::build(uint64_t up,uint64_t rtc,SystemMode mode,Transport transport,const std::array<ZoneState,8>& zones,const std::array<PressureState,2>& pressures,const HealthMonitor& health,const std::array<float,8>& temperatures_c,const std::array<bool,8>& temperature_valid,uint8_t temperature_count,float battery_voltage_v,float battery_current_a,float battery_power_w,bool battery_valid,const std::array<float,2>& pressure_values,const std::array<bool,2>& pressure_valid){
