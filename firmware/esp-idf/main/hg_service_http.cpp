...
esp_err_t ServiceHttp::factory_reset_post(httpd_req_t* request) {
    ...
    if (self == nullptr || self->access_control_ == nullptr) {
        httpd_resp_set_status(request, "503 Service Unavailable");
        return self != nullptr
            ? self->send_json(request, "{\"ok\":false,\"reason\":\"access_unavailable\"}")
            : ESP_FAIL;
    }
    ...
    const auto response = self->send_json(request,
        "{\"ok\":true,\"reason\":\"factory_reset_started\",\"rebooting\":true}");

    if (xTaskCreate(&delayed_restart, "hg_factory_reboot", 1536, nullptr, 5, nullptr) != pdPASS) {
        esp_restart();
    }
    return response;
}
