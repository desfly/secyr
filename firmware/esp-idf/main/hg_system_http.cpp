...
namespace {
SystemHttp* self_from(httpd_req_t* request) { return static_cast<SystemHttp*>(request->user_ctx); }
...
esp_err_t SystemHttp::register_handlers(httpd_handle_t server, hg::SystemModel* model,
    hg::SystemEventBus* bus, homeguard::AccessControl* access_control,
    const homeguard::UserZoneAccess* zone_access) {
    if (server == nullptr || model == nullptr || bus == nullptr || access_control == nullptr || zone_access == nullptr) return ESP_ERR_INVALID_ARG;
    server_ = server; model_ = model; bus_ = bus; access_control_ = access_control; zone_access_ = zone_access;
    ...
    for (const auto& route : routes) { const auto error=httpd_register_uri_handler(server_, &route); if (error!=ESP_OK) return error; }
    return ESP_OK;
}

esp_err_t SystemHttp::send_json(httpd_req_t* request, const char* body, std::size_t size) const {
    httpd_resp_set_type(request,"application/json"); return httpd_resp_send(request,body,static_cast<ssize_t>(size));
}

esp_err_t SystemHttp::status_get(httpd_req_t* request) {
    auto* self=self_from(request); if(self==nullptr||self->model_==nullptr||self->bus_==nullptr) return ESP_FAIL;
    const auto body=hg::system_status_json(*self->model_,*self->bus_); return self->send_json(request,body.c_str(),body.size());
}
esp_err_t SystemHttp::zones_get(httpd_req_t* request) {
    auto* self=self_from(request); if(self==nullptr||self->model_==nullptr) return ESP_FAIL;
    const auto body=hg::system_zones_json(*self->model_); return self->send_json(request,body.c_str(),body.size());
}
esp_err_t SystemHttp::outputs_get(httpd_req_t* request) {
    auto* self=self_from(request); if(self==nullptr||self->model_==nullptr) return ESP_FAIL;
    const auto body=hg::system_outputs_json(*self->model_); return self->send_json(request,body.c_str(),body.size());
}
esp_err_t SystemHttp::partitions_get(httpd_req_t* request) {
    auto* self=self_from(request); if(self==nullptr||self->model_==nullptr) return ESP_FAIL;
    const auto body=hg::system_partitions_json(*self->model_); return self->send_json(request,body.c_str(),body.size());
}
esp_err_t SystemHttp::events_get(httpd_req_t* request) {
    auto* self=self_from(request); if(self==nullptr) return ESP_FAIL; const auto body=self->events_json(); return self->send_json(request,body.c_str(),body.size());
}
esp_err_t SystemHttp::security_command_post(httpd_req_t* request) { auto* self=self_from(request); return self==nullptr?ESP_FAIL:self->handle_security_command(request); }

bool SystemHttp::zone_matrix_allows(std::string_view actor, hg::PartitionArmState target) const {
    if (model_ == nullptr || access_control_ == nullptr || zone_access_ == nullptr) return false;
    ...
    auto allows_zone = [&](std::uint16_t zone_id) {
        const auto* rule = zone_access_->rule(actor, zone_id);
        if (rule == nullptr || !rule->visible) return false;
        return target == hg::PartitionArmState::Disarmed ? rule->can_disarm : rule->can_arm;
    };
    if (partition != nullptr && partition->zone_count > 0U) {
        for (std::size_t i=0; i<partition->zone_count; ++i) if (!allows_zone(partition->zone_ids[i])) return false;
        return true;
    }
    for (std::size_t i=0; i<model_->zone_count(); ++i) {
        const auto* zone=model_->zone_at(i); if(zone!=nullptr && zone->enabled && !zone->bypassed && !allows_zone(zone->id)) return false;
    }
    return true;
}
...
esp_err_t SystemHttp::handle_security_command(httpd_req_t* request) {
    ...
    if(received<=0){httpd_resp_set_status(request,"400 Bad Request");return httpd_resp_send(request,"{\"ok\":false,\"reason\":\"read_failed\"}",-1);} body.resize(static_cast<std::size_t>(received));
    ...
    else {httpd_resp_set_status(request,"400 Bad Request");return httpd_resp_send(request,"{\"ok\":false,\"reason\":\"unsupported_command\"}",-1);}
    if(target!=hg::PartitionArmState::Alarm && !zone_matrix_allows(actor,target)){
        httpd_resp_set_status(request,"403 Forbidden"); return httpd_resp_send(request,"{\"ok\":false,\"reason\":\"zone_matrix_denied\"}",-1);
    }
...
esp_err_t SystemHttp::websocket(httpd_req_t* request){auto* self=self_from(request);if(self==nullptr)return ESP_FAIL;self->remember_client(httpd_req_to_sockfd(request));return ESP_OK;}
void SystemHttp::on_event(const hg::SystemEvent& event,void* context){auto* self=static_cast<SystemHttp*>(context);if(self==nullptr)return;self->record(event);self->broadcast(event);}
