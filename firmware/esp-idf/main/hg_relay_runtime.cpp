...
esp_err_t RelayRuntime::start(ZoneMonitor* zones)
{
    ...
    // Fail-safe first: all four relay lines are LOW before entering runtime.
...
{
    // Write every line so the software state can only advance after all four
    // physical GPIO writes succeed. Any partial write is immediately forced OFF.
