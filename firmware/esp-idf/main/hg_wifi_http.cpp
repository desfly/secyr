...
<section class="card wide"><h2>Домашня Wi‑Fi мережа</h2><div class="grid"><div><label>SSID</label><input id="ssid" autocomplete="off" placeholder="Назва Wi‑Fi"></div><div><label>Пароль</label><div class="password-wrap"><input id="password" type="password" autocomplete="new-password" placeholder="Пароль Wi‑Fi"><button id="passwordEye" class="password-eye" type="button" aria-label="Показати пароль" title="Показати пароль" onclick="togglePassword()"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12z"></path><circle cx="12" cy="12" r="2.5"></circle></svg></button></div></div></div><div class="row"><button type="button" onclick="scanWifi()">Пошук мереж Wi‑Fi</button><button type="button" onclick="saveWifi()">Зберегти та підключити</button><button type="button" class="secondary" onclick="refreshAll()">Оновити стан</button></div><div id="wifiMsg" class="msg muted"></div><div id="networks" class="nets"></div></section>
...
function togglePassword(){const p=$('password'),b=$('passwordEye'),show=p.type==='password';p.type=show?'text':'password';b.setAttribute('aria-label',show?'Приховати пароль':'Показати пароль');b.title=show?'Приховати пароль':'Показати пароль';b.style.opacity=show?'1':'0.72'}
