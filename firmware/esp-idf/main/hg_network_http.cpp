...
public:
    explicit AsyncRequestGuard(httpd_req_t* request) : request_(request) {}
    AsyncRequestGuard(const AsyncRequestGuard&) = delete;
    AsyncRequestGuard& operator=(const AsyncRequestGuard&) = delete;
    ~AsyncRequestGuard()
    {
        if (request_ != nullptr) (void)httpd_req_async_handler_complete(request_);
    }

    esp_err_t complete()
    {
        if (request_ == nullptr) return ESP_OK;
...
esp_err_t NetworkHttp::begin()
{
    ...
    // A candidate is never authoritative after a reboot. The committed record
    // remains the sole auto-connect source until a candidate reaches GOT_IP.
...
esp_err_t NetworkHttp::handle_connect(httpd_req_t* request)
{
    if (request->content_len == 0 || request->content_len > 384) {
        return send_error(request, "400 Bad Request", "invalid_body");
    }
    ...
    while (offset < body.size()) {
        ...
        if (received <= 0) {
            wipe(body);
            return send_error(request, "400 Bad Request", "read_failed");
        }
    ...
    std::string ssid;
    std::string password;
    if (!parse_json_string(body, "ssid", ssid) ||
...
esp_err_t NetworkHttp::dispatch_async(httpd_req_t* request,
                                      ...
                                      std::string password)
{
    if (candidate_pending_.load()) {
        wipe(password);
        return send_error(request, "409 Conflict", "network_operation_busy");
    }

    bool expected = false;
    if (!async_busy_.compare_exchange_strong(expected, true)) {
        wipe(password);
        return send_error(request, "409 Conflict", "network_operation_busy");
    }
    ...
    if (xTaskCreate(&NetworkHttp::async_task_entry,
                    operation == AsyncOperation::Scan ? "hg_wifi_scan" : "hg_wifi_connect",
...
void NetworkHttp::reconnect_task_entry(void* argument)
{
    ...
    if (!self->candidate_pending_.load() &&
        !self->async_busy_.load() &&
        self->sta_state_.load() == StaState::Error) {
        // Re-apply the committed record instead of trusting any transient RAM
        // configuration left behind by a failed candidate handover.
        self->restore_in_progress_.store(true);
        self->restore_active_connection();
    }

    self->reconnect_scheduled_.store(false);
    if (!self->candidate_pending_.load() && self->sta_state_.load() == StaState::Error) {
...
void NetworkHttp::candidate_timeout_task_entry(void* argument)
{
    ...
    if (self->candidate_generation_.load() == generation && self->candidate_pending_.exchange(false)) {
        self->cancel_candidate_and_restore_active("candidate_timeout");
    }
    vTaskDelete(nullptr);
}
...
{
    if (context.operation == AsyncOperation::Scan) process_scan(context);
    else process_connect(context);
}

void NetworkHttp::process_scan(AsyncContext& context)
{
    AsyncRequestGuard request_guard(context.request);
    const auto response_error = send_json(context.request, scan_json());
    if (response_error != ESP_OK) {
        ESP_LOGW(kTag, "Wi-Fi scan response failed: %s", esp_err_to_name(response_error));
    }
    const auto complete_error = request_guard.complete();
    if (complete_error != ESP_OK) {
        ESP_LOGW(kTag, "Wi-Fi scan async completion failed: %s", esp_err_to_name(complete_error));
    }
}
...
void NetworkHttp::process_connect(AsyncContext& context)
{
    ...
    // Persist only as a disposable candidate. The committed record is not
    // touched until the candidate is proven by IP_EVENT_STA_GOT_IP.
    ...
    const auto response_error = send_json(
        context.request,
        std::string{"{\"ok\":true,\"state\":\"handover_pending\",\"ssid\":\""} +
            json_escape(context.ssid) +
            "\",\"setupMode\":" +
            (context.setup_mode ? "true" : "false") +
            "}");
    const auto complete_error = request_guard.complete();

    // Never mutate STA transport if the HTTP acknowledgement did not finish.
    // This is the central socket-safety rule for Web UI initiated handovers.
    if (response_error != ESP_OK || complete_error != ESP_OK) {
        (void)clear_candidate_credentials();
        ESP_LOGW(kTag, "Wi-Fi handover cancelled because HTTP acknowledgement did not complete");
        return;
    }
    ...
    // Follow the ESP-IDF connection contract: disconnect the current STA first,
    // then replace the RAM-only station configuration, then connect once.
    ...
    if (!set_sta_config(context.ssid, context.password)) {
        if (candidate_pending_.exchange(false)) {
            cancel_candidate_and_restore_active("candidate_config_failed");
        }
        return;
    }
    ...
    if (connect_error != ESP_OK) {
        if (candidate_pending_.exchange(false)) {
            cancel_candidate_and_restore_active("connect_command_failed");
        }
        return;
    }

    if (!start_candidate_timeout(generation) && candidate_pending_.exchange(false)) {
        cancel_candidate_and_restore_active("timeout_guard_failed");
    }
}
...
bool NetworkHttp::start_candidate_timeout(std::uint32_t generation)
{
    ...
    if (xTaskCreate(&NetworkHttp::candidate_timeout_task_entry,
                    "hg_wifi_timeout",
                    kTimeoutTaskStackBytes,
                    context,
                    kWorkerTaskPriority,
                    nullptr) != pdPASS) {
        delete context;
...
void NetworkHttp::restore_active_connection()
{
    ...
    if (!set_sta_config(ssid, password)) {
        wipe(password);
        sta_state_.store(StaState::Error);
        restore_in_progress_.store(false);
        schedule_reconnect_retry();
...
void NetworkHttp::on_wifi_event(std::int32_t id, void* data)
{
    ...
    if (candidate_pending_.load()) {
        ...
        if (attempt > kCandidateReconnectAttempts) {
            if (candidate_pending_.exchange(false)) {
                cancel_candidate_and_restore_active("candidate_retries_exhausted");
            }
            return;
        }
        sta_state_.store(StaState::Connecting);
        if (esp_wifi_connect() != ESP_OK && candidate_pending_.exchange(false)) {
            cancel_candidate_and_restore_active("candidate_retry_command_failed");
        }
        return;
    }
    ...
    // A user-requested scan runs on a worker because it is blocking. Do not
    // start a competing connection scan underneath it; retry after the worker
    // releases the network operation gate.
    if (async_busy_.load()) {
        sta_state_.store(StaState::Error);
        schedule_reconnect_retry();
        return;
...
void NetworkHttp::schedule_reconnect_retry()
{
    ...
    if (xTaskCreate(&NetworkHttp::reconnect_task_entry,
                    "hg_wifi_retry",
                    kReconnectTaskStackBytes,
                    this,
                    kWorkerTaskPriority,
                    nullptr) != pdPASS) {
...
void NetworkHttp::on_ip_event(std::int32_t id, void*)
{
    ...
    if (candidate_pending_.load()) {
        ...
        if (!load_candidate_credentials(candidate_ssid, candidate_password)) {
            if (candidate_pending_.exchange(false)) {
                cancel_candidate_and_restore_active("candidate_record_missing");
            }
            return;
        }

        // GOT_IP can be queued from the old connection while the candidate
        // handover is starting. Never commit merely because an IP event exists;
        // the currently associated AP must be the candidate SSID.
        if (!current_ap_matches(candidate_ssid)) {
            wipe(candidate_password);
            return;
        }

        bool expected = true;
        if (!candidate_pending_.compare_exchange_strong(expected, false)) {
            wipe(candidate_password);
            return;
        }

        const bool committed = save_credentials(candidate_ssid, candidate_password);
        wipe(candidate_password);
        (void)clear_candidate_credentials();
        if (!committed) {
            restore_in_progress_.store(true);
    ...
    if (restore_in_progress_.load()) {
        ...
        if (!have_active || !current_ap_matches(active_ssid)) return;
        restore_in_progress_.store(false);
    }
...
bool NetworkHttp::set_sta_config(const std::string& ssid, const std::string& password)
{
    wifi_config_t sta{};
...
bool NetworkHttp::current_ap_matches(const std::string& ssid) const
{
    wifi_ap_record_t ap_info{};
    if (esp_wifi_sta_get_ap_info(&ap_info) != ESP_OK) return false;
    return ssid_from_bytes(ap_info.ssid, sizeof(ap_info.ssid)) == ssid;
}

bool NetworkHttp::apply_sta(const std::string& ssid, const std::string& password, bool persist)
{
    if (!initialized_ || ssid.empty() || ssid.size() > 32 ||
        password.size() > 64 || (!password.empty() && password.size() < 8)) {
        return false;
    }
    if (persist && !save_credentials(ssid, password)) return false;
    if (!set_sta_config(ssid, password)) return false;
...
{
    return load_credentials_from_key(kNvsKey, ssid, password);
}

bool NetworkHttp::save_credentials(const std::string& ssid, const std::string& password) const
{
    return save_credentials_to_key(kNvsKey, ssid, password);
}

bool NetworkHttp::clear_credentials() const
{
    return clear_credentials_key(kNvsKey);
}

bool NetworkHttp::load_candidate_credentials(std::string& ssid, std::string& password) const
{
    return load_credentials_from_key(kCandidateNvsKey, ssid, password);
}

bool NetworkHttp::save_candidate_credentials(const std::string& ssid, const std::string& password) const
{
    return save_credentials_to_key(kCandidateNvsKey, ssid, password);
}

bool NetworkHttp::clear_candidate_credentials() const
{
    return clear_credentials_key(kCandidateNvsKey);
}
...
bool NetworkHttp::load_credentials_from_key(const char* key, std::string& ssid, std::string& password) const
{
    ...
    if (error != ESP_OK ||
        ...
        record.ssid_length == 0 ||
        record.ssid_length > record.ssid.size() ||
...
bool NetworkHttp::save_credentials_to_key(const char* key,
...
std::string NetworkHttp::status_json() const
{
    ...
    std::string out = std::string{"{\"ok\":true,\"state\":\""} +
        state_name(state) +
