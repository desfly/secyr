...
esp_err_t start_http_server()
{
    ...
    const auto rollback_http = [](esp_err_t error, const char* stage) -> esp_err_t {
        ...
        ESP_LOGE(kTag, "HTTP bootstrap route registration failed at %s: %s; rolling back partial server", stage, esp_err_to_name(error));
    ...
    // Port 80 is bootstrap-only. Operational control, status, access, service,
    // build, cloud, hardware and telemetry routes live exclusively on HTTPS.
