...
{
    if (g_owner == nullptr || ctxt == nullptr || ctxt->op != BLE_GATT_ACCESS_OP_WRITE_CHR) {
    ...
    if (ble_hs_mbuf_to_flat(ctxt->om, value.data(), value.size(), &copied) != 0) {
        return BLE_ATT_ERR_UNLIKELY;
    }
...
{
    if (g_owner == nullptr || event == nullptr) return 0;
    switch (event->type) {
        case BLE_GAP_EVENT_CONNECT:
            if (event->connect.status == 0) {
                g_owner->connection_handle_ = event->connect.conn_handle;
                g_owner->notify_enabled_ = false;
                g_owner->reset_rx();
                ESP_LOGI(kTag, "Android BLE link connected; handle=%u", event->connect.conn_handle);
            } else {
        ...
        case BLE_GAP_EVENT_DISCONNECT:
            ...
            g_owner->connection_handle_ = BLE_HS_CONN_HANDLE_NONE;
            g_owner->notify_enabled_ = false;
            g_owner->reset_rx();
            (void)g_owner->advertise();
            break;
        ...
        case BLE_GAP_EVENT_SUBSCRIBE:
            if (event->subscribe.attr_handle == g_tx_value_handle) {
                g_owner->notify_enabled_ = event->subscribe.cur_notify != 0;
                ESP_LOGI(kTag, "BLE telemetry notifications %s", g_owner->notify_enabled_ ? "enabled" : "disabled");
            }
            break;
        default:
            break;
...
void stack_sync()
{
    ...
    if (ble_hs_id_infer_auto(0, &g_owner->own_address_type_) != 0) {
...
{
    if (device_name == nullptr || device_name[0] == '\0') return ESP_ERR_INVALID_ARG;
    if (g_owner != nullptr && g_owner != this) return ESP_ERR_INVALID_STATE;
    g_owner = this;
    ...
    if (xTaskCreate(host_task, "hg_ble_host", 4096, nullptr, 5, nullptr) != pdPASS) {
...
{
    if (data == nullptr || size < kHeaderSize || data[0] != kProtocolVersion) return BLE_ATT_ERR_UNLIKELY;
    const auto type = data[1];
    const auto message_id = static_cast<std::uint16_t>(data[2]) |
        (static_cast<std::uint16_t>(data[3]) << 8U);
    const auto index = data[4];
    const auto count = data[5];
    if (count == 0 || index >= count) return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;

    if (index == 0) {
        reset_rx();
        rx_type_ = type;
        rx_message_id_ = message_id;
        rx_expected_count_ = count;
    }
    if (type != rx_type_ || message_id != rx_message_id_ || count != rx_expected_count_ || index != rx_next_index_) {
        reset_rx();
        return BLE_ATT_ERR_UNLIKELY;
    }

    const auto payload_size = size - kHeaderSize;
    if (rx_payload_.size() + payload_size > kMaxMessageBytes) {
        reset_rx();
        return BLE_ATT_ERR_INVALID_ATTR_VALUE_LEN;
    }
    rx_payload_.append(reinterpret_cast<const char*>(data + kHeaderSize), payload_size);
    ++rx_next_index_;

    if (rx_next_index_ == rx_expected_count_) {
        const auto completed_type = rx_type_;
        const auto completed = std::move(rx_payload_);
        reset_rx();
        if (message_handler_ != nullptr) message_handler_(completed_type, completed, message_context_);
    }
    return 0;
}
...
esp_err_t BleTransport::notify_message(std::uint8_t type, const std::string& payload)
{
    ...
    const auto fragment_count = std::max<std::size_t>(1, (payload.size() + payload_limit - 1) / payload_limit);
    if (fragment_count > 255) return ESP_ERR_INVALID_SIZE;
    ...
    for (std::size_t index = 0; index < fragment_count; ++index) {
        ...
        const auto length = std::min(payload_limit, payload.size() - std::min(start, payload.size()));
        std::array<std::uint8_t, kMaxGattValue> frame{};
        frame[0] = kProtocolVersion;
        frame[1] = type;
        frame[2] = static_cast<std::uint8_t>(message_id & 0xffU);
        frame[3] = static_cast<std::uint8_t>((message_id >> 8U) & 0xffU);
        frame[4] = static_cast<std::uint8_t>(index);
        frame[5] = static_cast<std::uint8_t>(fragment_count);
        if (length != 0) std::memcpy(frame.data() + kHeaderSize, payload.data() + start, length);

        auto* om = ble_hs_mbuf_from_flat(frame.data(), kHeaderSize + length);
        if (om == nullptr) return ESP_ERR_NO_MEM;
        const int rc = ble_gatts_notify_custom(connection_handle_, g_tx_value_handle, om);
        if (rc != 0) return ESP_FAIL;
    }
    return ESP_OK;
}
