...
homeguard::HomeGuardConfigDocument valid_document()
{
    ...
    doc.zone_count = 2;
    doc.zones[0].id = 1; set_text(doc.zones[0].name, "Front door");
    doc.zones[0].type = homeguard::ConfigZoneType::EntryExit; doc.zones[0].entry_delay_sec = 30; doc.zones[0].exit_delay_sec = 45;
    doc.zones[1].id = 2; set_text(doc.zones[1].name, "Kitchen flood");
    doc.zones[1].type = homeguard::ConfigZoneType::Flood24h; doc.zones[1].entry_delay_sec = 0; doc.zones[1].exit_delay_sec = 0;

    doc.output_count = 2;
    doc.outputs[0].id = 1; set_text(doc.outputs[0].name, "Siren"); doc.outputs[0].type = homeguard::ConfigOutputType::Siren;
    doc.outputs[1].id = 2; set_text(doc.outputs[1].name, "Water valve"); doc.outputs[1].type = homeguard::ConfigOutputType::Valve;

    doc.user_count = 3;
    set_text(doc.users[0].id, "admin"); set_text(doc.users[0].name, "Administrator"); doc.users[0].role = homeguard::AccessRole::Admin;
    set_text(doc.users[1].id, "user1"); set_text(doc.users[1].name, "User 1"); doc.users[1].role = homeguard::AccessRole::User;
    set_text(doc.users[2].id, "guest"); set_text(doc.users[2].name, "Guest"); doc.users[2].role = homeguard::AccessRole::Guest;
