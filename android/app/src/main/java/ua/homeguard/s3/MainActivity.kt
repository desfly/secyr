...
class MainActivity : ComponentActivity() {
    ...
    private val settingsBackupLauncher = registerForActivityResult(ActivityResultContracts.CreateDocument("application/json")) { uri ->
        ...
        backupStatus.value = if (result.isSuccess) {
            "Backup налаштувань збережено"
        } else {
            "Помилка backup: ${result.exceptionOrNull()?.message ?: "write"}"
        }
        pendingSettingsBackupText = ""
    }
        ...
        lifecycleScope.launch {
            discovery.devices.collect { found ->
                found.forEach { device ->
                    ...
                    if (manual != null) {
                        if (registeredDevices.reconcileManual(manual.deviceId, device) && settings.settings.value.deviceId == manual.deviceId) {
            ...
            commandStatus.value = result.fold(
                { reply ->
                    if (reply.accepted || reply.duplicate) {
                        ...
                        if (
                            reply.code.contains("unauthorized", true) ||
                            reply.code.contains("credential", true) ||
                            reply.code.contains("rate", true)
                        ) {
                            accessSession.value = null
                        }
    ...
    private fun launchQrScanner() {
        qrScanner.launch(
            ScanOptions()
                .setPrompt("Скануйте QR HomeGuard-S3")
